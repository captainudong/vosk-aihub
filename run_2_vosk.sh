...
###
###  local/data_prep.sh $database/LibriSpeech/train-clean-5 data/aihub2
###  local/data_prep.sh $database/LibriSpeech/dev-clean-2 data/aihub2test
###fi
...
if [ $stage -le 3 ]; then
  steps/train_mono.sh --nj 10 --cmd "$train_cmd" \
    data/aihub2 data/lang exp/mono

  steps/align_si.sh  --nj 10 --cmd "$train_cmd" \
    data/aihub2 data/lang exp/mono exp/mono_ali

  steps/train_lda_mllt.sh  --cmd "$train_cmd" \
    2000 10000 data/aihub2 data/lang exp/mono_ali exp/tri1

  steps/align_si.sh --nj 10 --cmd "$train_cmd" \
    data/aihub2 data/lang exp/tri1 exp/tri1_ali

  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    2500 15000 data/aihub2 data/lang exp/tri1_ali exp/tri2

  steps/align_si.sh  --nj 10 --cmd "$train_cmd" \
    data/aihub2 data/lang exp/tri2 exp/tri2_ali

  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    2500 20000 data/aihub2 data/lang exp/tri2_ali exp/tri3

  steps/align_si.sh  --nj 10 --cmd "$train_cmd" \
    data/aihub2 data/lang exp/tri3 exp/tri3_ali
fi
