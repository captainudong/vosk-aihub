...
###
###  local/data_prep.sh $database/LibriSpeech/train-clean-5 data/aihub1
###  local/data_prep.sh $database/LibriSpeech/dev-clean-2 data/aihub1test
###fi
...
if [ $stage -le 3 ]; then
  steps/train_mono.sh --nj 10 --cmd "$train_cmd" \
    data/aihub1 data/lang exp/mono

  steps/align_si.sh  --nj 10 --cmd "$train_cmd" \
    data/aihub1 data/lang exp/mono exp/mono_ali

  steps/train_lda_mllt.sh  --cmd "$train_cmd" \
    2000 10000 data/aihub1 data/lang exp/mono_ali exp/tri1

  steps/align_si.sh --nj 10 --cmd "$train_cmd" \
    data/aihub1 data/lang exp/tri1 exp/tri1_ali

  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    2500 15000 data/aihub1 data/lang exp/tri1_ali exp/tri2

  steps/align_si.sh  --nj 10 --cmd "$train_cmd" \
    data/aihub1 data/lang exp/tri2 exp/tri2_ali

  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    2500 20000 data/aihub1 data/lang exp/tri2_ali exp/tri3

  steps/align_si.sh  --nj 10 --cmd "$train_cmd" \
    data/aihub1 data/lang exp/tri3 exp/tri3_ali
fi
...
if [ $stage -le 5 ]; then
  ...
  ###for task in test; do
  for task in aihub1test; do

    echo -----
    echo steps/make_mfcc.sh --cmd "$train_cmd" --nj 10 data/$task exp/make_mfcc/$task $mfcc
    steps/make_mfcc.sh --cmd "$train_cmd" --nj 10 data/$task exp/make_mfcc/$task $mfcc
    echo -----
    echo steps/compute_cmvn_stats.sh data/$task exp/make_mfcc/$task $mfcc
    steps/compute_cmvn_stats.sh data/$task exp/make_mfcc/$task $mfcc
